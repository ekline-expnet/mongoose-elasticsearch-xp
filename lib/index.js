...
module.exports = function (schema, options, version = 6) {
  ...
  function esOptions() {
    ...
    if (options.version < 7 && (!options.index || !options.type)) {
      throw new Error(
        options.index
...
function _indexDoc(id, body, esOptions, resolve, reject, update) {
  esOptions.client[update ? 'update' : 'index'](
    ...
    (err /* , { resp } */) => {
      let errStatus;
      if (esOptions.version < 6) {
        errStatus = err.status;
      } else if (err.meta && err.meta.body) {
        errStatus = err.meta.body.status;
      }
      if (update && errStatus === 404) {
